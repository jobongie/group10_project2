...
@app.route("/scatter")
def scatter():
    pmobs_df = table_df[['YEAR', 'SECTOR', 'INDUSTRY', "NUMBER_OF_FIRMS", "BUSINESS_CLASSIFICATION",'EMPLOYMENT','ANNUAL_PAYROLL']].copy()
    remove_data = ['Industries not classified', 'X']
    pmobs_df = pmobs_df.loc[~pmobs_df["INDUSTRY"].isin(remove_data)]
    pmobg_df = pmobs_df.groupby(['YEAR', 'SECTOR', 'INDUSTRY', "BUSINESS_CLASSIFICATION" ])
